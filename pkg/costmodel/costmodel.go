...
const (
	queryRAMRequestsStr = `avg(
		label_replace(
			label_replace(
				avg(
					count_over_time(kube_pod_container_resource_requests{resource="memory", unit="byte", container!="",container!="POD", node!="", cluster="udaan-k8s0"}[%s] %s)
					*
					avg_over_time(kube_pod_container_resource_requests{resource="memory", unit="byte", container!="",container!="POD", node!="", cluster="udaan-k8s0"}[%s] %s)
	...
	queryRAMUsageStr = `sort_desc(
		avg(
			label_replace(count_over_time(container_memory_working_set_bytes{container_name!="",container_name!="POD", instance!="", cluster="udaan-k8s0"}[%s] %s), "node", "$1", "instance","(.+)")
			*
			label_replace(avg_over_time(container_memory_working_set_bytes{container_name!="",container_name!="POD", instance!="", cluster="udaan-k8s0"}[%s] %s), "node", "$1", "instance","(.+)")
		) by (namespace,container_name,pod_name,node,%s)
	)`
	queryCPURequestsStr = `avg(
		label_replace(
			label_replace(
				avg(
					count_over_time(kube_pod_container_resource_requests{resource="cpu", unit="core", container!="",container!="POD", node!="", cluster="udaan-k8s0"}[%s] %s)
					*
					avg_over_time(kube_pod_container_resource_requests{resource="cpu", unit="core", container!="",container!="POD", node!="", cluster="udaan-k8s0"}[%s] %s)
				) by (namespace,container,pod,node,%s) , "container_name","$1","container","(.+)"
			), "pod_name","$1","pod","(.+)"
		)
	) by (namespace,container_name,pod_name,node,%s)`
	queryCPUUsageStr = `avg(
		label_replace(
		rate(
			container_cpu_usage_seconds_total{container_name!="",container_name!="POD",instance!="", cluster="udaan-k8s0"}[%s] %s
		) , "node", "$1", "instance", "(.+)"
		)
	) by (namespace,container_name,pod_name,node,%s)`
	queryGPURequestsStr = `avg(
		label_replace(
			label_replace(
				avg(
					count_over_time(kube_pod_container_resource_requests{resource="nvidia_com_gpu", container!="",container!="POD", node!="", cluster="udaan-k8s0"}[%s] %s)
					*
					avg_over_time(kube_pod_container_resource_requests{resource="nvidia_com_gpu", container!="",container!="POD", node!="", cluster="udaan-k8s0"}[%s] %s)
					* %f
				) by (namespace,container,pod,node,%s) , "container_name","$1","container","(.+)"
			), "pod_name","$1","pod","(.+)"
		)
	) by (namespace,container_name,pod_name,node,%s)
	* on (pod_name, namespace, %s) group_left(container) label_replace(avg(avg_over_time(kube_pod_status_phase{phase="Running", cluster="udaan-k8s0"}[%s] %s)) by (pod,namespace,%s), "pod_name","$1","pod","(.+)")`
	queryPVRequestsStr = `avg(avg(kube_persistentvolumeclaim_info{volumename != "", cluster="udaan-k8s0"}) by (persistentvolumeclaim, storageclass, namespace, volumename, %s, kubernetes_node)
	*
	on (persistentvolumeclaim, namespace, %s, kubernetes_node) group_right(storageclass, volumename)
	sum(kube_persistentvolumeclaim_resource_requests_storage_bytes{cluster="udaan-k8s0"}) by (persistentvolumeclaim, namespace, %s, kubernetes_node, kubernetes_name)) by (persistentvolumeclaim, storageclass, namespace, %s, volumename, kubernetes_node)`
	...
	queryRAMAllocationByteHours = `
		label_replace(label_replace(
			sum(
				sum_over_time(container_memory_allocation_bytes{container!="",container!="POD", node!="", cluster="udaan-k8s0"}[%s])
	...
	queryCPUAllocationVCPUHours = `
		label_replace(label_replace(
			sum(
				sum_over_time(container_cpu_allocation{container!="",container!="POD", node!="", cluster="udaan-k8s0"}[%s])
			) by (namespace,container,pod,node,%s) * %f / 60 / 60
		, "container_name","$1","container","(.+)"), "pod_name","$1","pod","(.+)")`
	...
	queryPVCAllocationFmt     = `sum(sum_over_time(pod_pvc_allocation{cluster="udaan-k8s0"}[%s])) by (%s, namespace, pod, persistentvolume, persistentvolumeclaim) * %f/60/60`
	queryPVHourlyCostFmt      = `avg_over_time(pv_hourly_cost{cluster="udaan-k8s0"}[%s])`
	queryNSLabels             = `avg_over_time(kube_namespace_labels{cluster="udaan-k8s0"}[%s])`
	queryPodLabels            = `avg_over_time(kube_pod_labels{cluster="udaan-k8s0"}[%s])`
	queryNSAnnotations        = `avg_over_time(kube_namespace_annotations{cluster="udaan-k8s0"}[%s])`
	queryPodAnnotations       = `avg_over_time(kube_pod_annotations{cluster="udaan-k8s0"}[%s])`
	queryDeploymentLabels     = `avg_over_time(deployment_match_labels{cluster="udaan-k8s0"}[%s])`
	queryStatefulsetLabels    = `avg_over_time(statefulSet_match_labels{cluster="udaan-k8s0"}[%s])`
	queryPodDaemonsets        = `sum(kube_pod_owner{owner_kind="DaemonSet", cluster="udaan-k8s0"}) by (namespace,pod,owner_name,%s)`
	queryPodJobs              = `sum(kube_pod_owner{owner_kind="Job", cluster="udaan-k8s0"}) by (namespace,pod,owner_name,%s)`
	queryServiceLabels        = `avg_over_time(service_selector_labels{cluster="udaan-k8s0"}[%s])`
	queryZoneNetworkUsage     = `sum(increase(kubecost_pod_network_egress_bytes_total{internet="false", sameZone="false", sameRegion="true", cluster="udaan-k8s0"}[%s] %s)) by (namespace,pod_name,%s) / 1024 / 1024 / 1024`
	queryRegionNetworkUsage   = `sum(increase(kubecost_pod_network_egress_bytes_total{internet="false", sameZone="false", sameRegion="false", cluster="udaan-k8s0"}[%s] %s)) by (namespace,pod_name,%s) / 1024 / 1024 / 1024`
	queryInternetNetworkUsage = `sum(increase(kubecost_pod_network_egress_bytes_total{internet="true", cluster="udaan-k8s0"}[%s] %s)) by (namespace,pod_name,%s) / 1024 / 1024 / 1024`
	normalizationStr          = `max(count_over_time(kube_pod_container_resource_requests{resource="memory", unit="byte", cluster="udaan-k8s0"}[%s] %s))`
)
...
func findDeletedNodeInfo(cli prometheusClient.Client, missingNodes map[string]*costAnalyzerCloud.Node, window, offset string) error {
	if len(missingNodes) > 0 {
		...
		queryHistoricalCPUCost := fmt.Sprintf(`avg(avg_over_time(node_cpu_hourly_cost{cluster="udaan-k8s0"}[%s] %s)) by (node, instance, %s)`, window, offsetStr, env.GetPromClusterLabel())
		queryHistoricalRAMCost := fmt.Sprintf(`avg(avg_over_time(node_ram_hourly_cost{cluster="udaan-k8s0"}[%s] %s)) by (node, instance, %s)`, window, offsetStr, env.GetPromClusterLabel())
		queryHistoricalGPUCost := fmt.Sprintf(`avg(avg_over_time(node_gpu_hourly_cost{cluster="udaan-k8s0"}[%s] %s)) by (node, instance, %s)`, window, offsetStr, env.GetPromClusterLabel())
