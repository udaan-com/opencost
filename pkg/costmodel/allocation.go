...
const (
	queryFmtPods                     = `avg(kube_pod_container_status_running{cluster="udaan-k8s0"}) by (pod, namespace, %s)[%s:%s]`
	queryFmtPodsUID                  = `avg(kube_pod_container_status_running{cluster="udaan-k8s0"}) by (pod, namespace, uid, %s)[%s:%s]`
	queryFmtRAMBytesAllocated        = `avg(avg_over_time(container_memory_allocation_bytes{container!="", container!="POD", node!="", cluster="udaan-k8s0"}[%s])) by (container, pod, namespace, node, %s, provider_id)`
	queryFmtRAMRequests              = `avg(avg_over_time(kube_pod_container_resource_requests{resource="memory", unit="byte", container!="", container!="POD", node!="", cluster="udaan-k8s0"}[%s])) by (container, pod, namespace, node, %s)`
	queryFmtRAMUsageAvg              = `avg(avg_over_time(container_memory_working_set_bytes{container!="", container_name!="POD", container!="POD", cluster="udaan-k8s0"}[%s])) by (container_name, container, pod_name, pod, namespace, instance, %s)`
	queryFmtRAMUsageMax              = `max(max_over_time(container_memory_working_set_bytes{container!="", container_name!="POD", container!="POD", cluster="udaan-k8s0"}[%s])) by (container_name, container, pod_name, pod, namespace, instance, %s)`
	queryFmtCPUCoresAllocated        = `avg(avg_over_time(container_cpu_allocation{container!="", container!="POD", node!="", cluster="udaan-k8s0"}[%s])) by (container, pod, namespace, node, %s)`
	queryFmtCPURequests              = `avg(avg_over_time(kube_pod_container_resource_requests{resource="cpu", unit="core", container!="", container!="POD", node!="", cluster="udaan-k8s0"}[%s])) by (container, pod, namespace, node, %s)`
	queryFmtCPUUsageAvg              = `avg(rate(container_cpu_usage_seconds_total{container!="", container_name!="POD", container!="POD", cluster="udaan-k8s0"}[%s])) by (container_name, container, pod_name, pod, namespace, instance, %s)`
	queryFmtCPUUsageMax              = `max(rate(container_cpu_usage_seconds_total{container!="", container_name!="POD", container!="POD", cluster="udaan-k8s0"}[%s])) by (container_name, container, pod_name, pod, namespace, instance, %s)`
	queryFmtGPUsRequested            = `avg(avg_over_time(kube_pod_container_resource_requests{resource="nvidia_com_gpu", container!="",container!="POD", node!="", cluster="udaan-k8s0"}[%s])) by (container, pod, namespace, node, %s)`
	queryFmtGPUsAllocated            = `avg(avg_over_time(container_gpu_allocation{container!="", container!="POD", node!="", cluster="udaan-k8s0"}[%s])) by (container, pod, namespace, node, %s)`
	queryFmtNodeCostPerCPUHr         = `avg(avg_over_time(node_cpu_hourly_cost{cluster="udaan-k8s0"}[%s])) by (node, %s, instance_type, provider_id)`
	queryFmtNodeCostPerRAMGiBHr      = `avg(avg_over_time(node_ram_hourly_cost{cluster="udaan-k8s0"}[%s])) by (node, %s, instance_type, provider_id)`
	queryFmtNodeCostPerGPUHr         = `avg(avg_over_time(node_gpu_hourly_cost{cluster="udaan-k8s0"}[%s])) by (node, %s, instance_type, provider_id)`
	queryFmtNodeIsSpot               = `avg_over_time(kubecost_node_is_spot{cluster="udaan-k8s0"}[%s])`
	queryFmtPVCInfo                  = `avg(kube_persistentvolumeclaim_info{volumename != "", cluster="udaan-k8s0"}) by (persistentvolumeclaim, storageclass, volumename, namespace, %s)[%s:%s]`
	queryFmtPVBytes                  = `avg(avg_over_time(kube_persistentvolume_capacity_bytes{cluster="udaan-k8s0"}[%s])) by (persistentvolume, %s)`
	queryFmtPodPVCAllocation         = `avg(avg_over_time(pod_pvc_allocation{cluster="udaan-k8s0"}[%s])) by (persistentvolume, persistentvolumeclaim, pod, namespace, %s)`
	queryFmtPVCBytesRequested        = `avg(avg_over_time(kube_persistentvolumeclaim_resource_requests_storage_bytes{cluster="udaan-k8s0"}[%s])) by (persistentvolumeclaim, namespace, %s)`
	queryFmtPVCostPerGiBHour         = `avg(avg_over_time(pv_hourly_cost{cluster="udaan-k8s0"}[%s])) by (volumename, %s)`
	queryFmtNetZoneGiB               = `sum(increase(kubecost_pod_network_egress_bytes_total{internet="false", sameZone="false", sameRegion="true", cluster="udaan-k8s0"}[%s])) by (pod_name, namespace, %s) / 1024 / 1024 / 1024`
	queryFmtNetZoneCostPerGiB        = `avg(avg_over_time(kubecost_network_zone_egress_cost{cluster="udaan-k8s0"}[%s])) by (%s)`
	queryFmtNetRegionGiB             = `sum(increase(kubecost_pod_network_egress_bytes_total{internet="false", sameZone="false", sameRegion="false", cluster="udaan-k8s0"}[%s])) by (pod_name, namespace, %s) / 1024 / 1024 / 1024`
	queryFmtNetRegionCostPerGiB      = `avg(avg_over_time(kubecost_network_region_egress_cost{cluster="udaan-k8s0"}[%s])) by (%s)`
	queryFmtNetInternetGiB           = `sum(increase(kubecost_pod_network_egress_bytes_total{internet="true", cluster="udaan-k8s0"}[%s])) by (pod_name, namespace, %s) / 1024 / 1024 / 1024`
	queryFmtNetInternetCostPerGiB    = `avg(avg_over_time(kubecost_network_internet_egress_cost{cluster="udaan-k8s0"}[%s])) by (%s)`
	queryFmtNetReceiveBytes          = `sum(increase(container_network_receive_bytes_total{pod!="", container="POD", cluster="udaan-k8s0"}[%s])) by (pod_name, pod, namespace, %s)`
	queryFmtNetTransferBytes         = `sum(increase(container_network_transmit_bytes_total{pod!="", container="POD", cluster="udaan-k8s0"}[%s])) by (pod_name, pod, namespace, %s)`
	queryFmtNamespaceLabels          = `avg_over_time(kube_namespace_labels{cluster="udaan-k8s0"}[%s])`
	queryFmtNamespaceAnnotations     = `avg_over_time(kube_namespace_annotations{cluster="udaan-k8s0"}[%s])`
	queryFmtPodLabels                = `avg_over_time(kube_pod_labels{cluster="udaan-k8s0"}[%s])`
	queryFmtPodAnnotations           = `avg_over_time(kube_pod_annotations{cluster="udaan-k8s0"}[%s])`
	queryFmtServiceLabels            = `avg_over_time(service_selector_labels{cluster="udaan-k8s0"}[%s])`
	queryFmtDeploymentLabels         = `avg_over_time(deployment_match_labels{cluster="udaan-k8s0"}[%s])`
	queryFmtStatefulSetLabels        = `avg_over_time(statefulSet_match_labels{cluster="udaan-k8s0"}[%s])`
	queryFmtDaemonSetLabels          = `sum(avg_over_time(kube_pod_owner{owner_kind="DaemonSet", cluster="udaan-k8s0"}[%s])) by (pod, owner_name, namespace, %s)`
	queryFmtJobLabels                = `sum(avg_over_time(kube_pod_owner{owner_kind="Job", cluster="udaan-k8s0"}[%s])) by (pod, owner_name, namespace ,%s)`
	queryFmtPodsWithReplicaSetOwner  = `sum(avg_over_time(kube_pod_owner{owner_kind="ReplicaSet", cluster="udaan-k8s0"}[%s])) by (pod, owner_name, namespace ,%s)`
	queryFmtReplicaSetsWithoutOwners = `avg(avg_over_time(kube_replicaset_owner{owner_kind="<none>", owner_name="<none>", cluster="udaan-k8s0"}[%s])) by (replicaset, namespace, %s)`
	queryFmtLBCostPerHr              = `avg(avg_over_time(kubecost_load_balancer_cost{cluster="udaan-k8s0"}[%s])) by (namespace, service_name, %s)`
	queryFmtLBActiveMins             = `count(kubecost_load_balancer_cost{cluster="udaan-k8s0"}) by (namespace, service_name, %s)[%s:%s]`
)
